...
def get_color(i):
    colors = ['darkorange', 'deepskyblue', 'darkviolet', 'cyan', 'magenta', 'black']
    return colors[i]
...
datafilename = 'impact-on-accuracy-varying-pruning-ratio'
yaxis_legend = 'Recall@1'
...
with open(datafilepath + datafilename + '.csv') as f:
    ...
    pylab.xlim([-0.05,0.75])
    pylab.ylim([0.50,1.02])
    # plt.title(title)
    # plt.legend()
    # legend_properties = {'weight':'bold'}
    legend = plt.legend(bbox_to_anchor=(0.55,0.06), loc='lower right', ncol=1, frameon=False)
    plt.tight_layout()
